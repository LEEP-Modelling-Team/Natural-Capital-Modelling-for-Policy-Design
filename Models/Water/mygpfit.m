function [parmhat,parmci] = mygpfit(x,parmbase,alpha,options)
...
parmhat = parmbase;
% lnsigma = log(parmbase(2));

% Maximize the log-likelihood with respect to k and lnsigma.
parmshapediff = 0;
%[parmhat,~,err,output] = fminsearch(@negloglike,parmhat,options,x,lnsigma);
[parmshapediff,~,err,output] = fminsearch(@negloglike,parmshapediff,options,x,parmbase);
            fprintf('        shape diff: %0.5f \n', parmshapediff)

parmhat(1) = parmhat(1) + parmshapediff;
...
function nll = negloglike(parmshapediff, data, parmbase)
% Negative log-likelihood for the GP (log(sigma) parameterization).
k       = parmbase(1) + parmshapediff;
sigma   = parmbase(2);
lnsigma = log(sigma);
