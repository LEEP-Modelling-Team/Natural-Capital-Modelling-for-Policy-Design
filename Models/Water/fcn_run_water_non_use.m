function es_water_non_use = fcn_run_water_non_use(water_non_use_data_folder, es_water)
    ...
    % --------------------------------------
    NEVO_water_non_use_data_mat = strcat(water_non_use_data_folder, 'NEVO_NonUseWQ_data.mat');
    ...
    [input_srcid_ind, input_srcid_idx] = ismember(NonUseWQ.src_id, cell2table(es_water.src_id, 'VariableNames', {'src_id'}));
    input_srcid_idx                    = input_srcid_idx(input_srcid_ind);
    
    % Reduce data down to these river cells
    NonUseWQ.src_id = NonUseWQ.src_id(input_srcid_ind, :);
    NonUseWQ.river_cells_1400_baseline = NonUseWQ.river_cells_1400_baseline(input_srcid_ind, :);
    NonUseWQ.nutr_init_class = NonUseWQ.nutr_init_class(input_srcid_ind, :);
    NonUseWQ.wfd_init_class = NonUseWQ.wfd_init_class(input_srcid_ind, :);
    ...
    [lsoa_ind_mat, lsoa_idx_mat] = ismember(NonUseWQ.closest_idx, river_cell_idx);
    lsoa_ind = any(lsoa_ind_mat); % logical of lsoas affected
    
    % Reduce data down to these lsoas
    NonUseWQ.lsoa_data = NonUseWQ.lsoa_data(lsoa_ind, :);
    NonUseWQ.cell_dist_500 = NonUseWQ.cell_dist_500(lsoa_ind, :);
    NonUseWQ.closest_idx = NonUseWQ.closest_idx(:, lsoa_ind);
    ...
    for i = 1:4
        ...
        lookup_table_idx = fcn_get_wfd_lookup_index(NonUseWQ.nutr_init_class(:,i), nutr_new_class(:,i), NonUseWQ.wfd_init_class(:,i));
        switch_prob = table2array(NonUseWQ.wfd_lookup_table(lookup_table_idx, :));
        ...
        WQual0 = [NonUseWQ.wfd_init_class(:,i) == 1, NonUseWQ.wfd_init_class(:,i) == 2, NonUseWQ.wfd_init_class(:,i) == 3, (NonUseWQ.wfd_init_class(:,i) == 4) | (NonUseWQ.wfd_init_class(:,i) == 5)];
        ...
        for j = 1:num_lsoa
            ...
            DistdeltaNU = NonUseWQ.cell_dist_500(j, lsoa_ind_mat(:, j)) .^ NonUseWQ.deltaNU;
            ...
            if river_length_scale_flag
                ...
                DistdeltaNU = DistdeltaNU .* NonUseWQ.river_cells_1400_baseline.river_length(lsoa_river_cells_order{j})' / 1000;
            end

            % Extract water quality classes/probability for necessary river cells
            % Change the ordering to be consistent with distances
            % Multiply by beta parameter
            XbQNU0 = WQual0(lsoa_river_cells_order{j}, 2:4) * NonUseWQ.betaQNU;
            XbQNU1 = WQual1(lsoa_river_cells_order{j}, 2:4) * NonUseWQ.betaQNU;
        ...
        es_water_non_use.(['value_ann', decade_string{i}]) = sum(NonUseWQ.lsoa_data.hhld .* (vCENU1 - vCENU0) / - NonUseWQ.deltaU);
