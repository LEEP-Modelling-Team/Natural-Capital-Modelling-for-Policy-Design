function rec_all_options = fcn_run_recreation_all_options(dorecmodel, cell_info, MP, conn)
    ...
    if dorecmodel
        ...
        % model
        sqlquery = ['SELECT arable_ha_20, grass_ha_20 FROM nevo_explore.explore_2km WHERE new2kid IN ', cell_info.new2kid_string];
...
%         rec_all_options.ar_sng_d_sng   = array2table([cell_info.new2kid, rec_ar_sng_d_sng], 'VariableNames', rec_var_names);
        ...
        %  =============================================
        save('Recreation Data/rec_vis_val_save.mat', 'rec_all_options', '-mat', '-v6');
        
    else
        load('Recreation Data/rec_vis_val_save.mat', 'rec_all_options');
    end
