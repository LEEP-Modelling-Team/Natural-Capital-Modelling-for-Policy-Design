%% RUN FORESTRY
%  ============
function es_forestry = fcn_run_forestry(forest_data_folder, ...
    ...
    % disturbed (arable) soils
    prop_grass    = ((mean(es_agriculture.pgrass_ha(:,1:10),2) + mean(es_agriculture.rgraz_ha(:,1:10),2)))./PV.farm_ha;
    % For cells with no farmland this is NaN, set to 0
    prop_grass(isnan(prop_grass)) = 0;
...
%         ForestGHG.landuse_base_cells = ForestGHG.landuse_base_cells(input_cell_idx_ghg,:);
        ...
        if wood_area_increased
            ...
            % If woodland has increased then must be the case that one or
            % both or semi-natural grassland or agriculture have decreased.
            
            % Areas of agric and sn-grass loss associated with woodland expansion:
            agric_loss_ha   = -agric_new_ha .* (agric_new_ha < 0) .* (wood_gain_ha > 0);
            sngrass_loss_ha = -sngrass_new_ha .* (sngrass_new_ha < 0) .* (wood_gain_ha > 0);
            
            % Transfer permanent grassland and semi-natural grassland to non-arable:
            sngrass_loss_ha = sngrass_loss_ha + agric_loss_ha .* prop_grass;
            agric_loss_ha   = agric_loss_ha .* (1 - prop_grass);
            
            % Woodland expansion where displacing just arable or just non-arable
            arable2wood_ha_cell  = wood_gain_ha .* (agric_loss_ha > 0)   .* (sngrass_loss_ha == 0);
            narable2wood_ha_cell = wood_gain_ha .* (sngrass_loss_ha > 0) .* (agric_loss_ha == 0);
            
            % Woodland expansion where displacing a mix of just arable & non-arable
            arable2wood_ha_cell  = wood_gain_ha .* agric_loss_ha./(agric_loss_ha + sngrass_loss_ha)   .* (sngrass_loss_ha > 0) .* (agric_loss_ha > 0);
            narable2wood_ha_cell = wood_gain_ha .* sngrass_loss_ha./(agric_loss_ha + sngrass_loss_ha) .* (sngrass_loss_ha > 0) .* (agric_loss_ha > 0);
            % For cells with no farmland or semi-natural grassland this is NaN, set to zero 
            arable2wood_ha_cell(isnan(arable2wood_ha_cell)) = 0;
            narable2wood_ha_cell(isnan(narable2wood_ha_cell)) = 0;
    ...
    for i = 1:height(ForestTimber.SpeciesCode)
        ...
        es_forestry.Timber.FixedCost.(species)  = npv_fxcst .* (PV.wood_ha > 0);
        ...
        if MP.run_ghg        
            ...
            es_forestry.TimberC.QntYrUB.(species)  = PV.wood_ha .* ((TimberC_qntUB_data_yrs + TimberC_qntUB_final_yrs) ./ 50);
